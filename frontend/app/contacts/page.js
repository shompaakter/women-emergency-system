...
export default function TrustedContacts() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    } catch (e) { console.error('fetch contacts error:', e.message); }
  ...
  const handleSubmit = async e => {
    ...
    // ── Validation — only name, phone, relation required ──
    if (!form.contactName.trim()) return setError('Full name is required.');
    if (!form.contactPhone.trim()) return setError('Phone number is required.');
    if (!form.relation) return setError('Please select a relationship.');
    if (!editId && contacts.length >= 5) return setError('Maximum 5 trusted contacts allowed.');
    ...
    try {
      ...
      if (editId) {
        await axios.put(`${API_URL}/api/contacts/${editId}`, payload, {
          headers: { Authorization: `Bearer ${getToken()}` },
        });
        setSuccess('Contact updated successfully.');
      } else {
        await axios.post(`${API_URL}/api/contacts`, payload, {
          headers: { Authorization: `Bearer ${getToken()}` },
        });
      ...
      setError(msg);
      console.error('contact save error:', err.response?.data);
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Add up to 5 contacts. SOS alert email will be sent to them.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5">
                Email Address <span className="text-pink-500">★ Required for SOS alerts</span>
              </label>
              <input
                type="email"
                name="email"
                ...
                className="w-full px-4 py-3 rounded-xl border border-pink-200 bg-pink-50/20 focus:outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100 text-sm transition-all"
              />
              {!form.email && (
                <p className="text-xs text-amber-600 mt-1">
                  ⚠️ Without email, SOS alert cannot be sent to this contact.
                </p>
              )}
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5">
                Relationship <span className="text-pink-500">*</span>
              </label>
              <select
                name="relation"
                value={form.relation}
                onChange={handleChange}
                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:border-pink-400 text-sm bg-white text-gray-700 transition-all"
              >
            ...
            <div className="space-y-3">
              {contacts.map(c => (
                <div key={c._id} className="bg-white rounded-2xl border border-gray-100 hover:border-pink-100 p-4 flex items-center gap-4 transition-colors">
                  <div className="w-11 h-11 rounded-full bg-linear-to-br from-pink-100 to-orange-100 flex items-center justify-center text-pink-700 font-bold text-base shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {c.email
                      ? <p className="text-xs text-pink-500 mt-0.5">📧 {c.email}</p>
                      : <p className="text-xs text-red-400 mt-0.5">⚠️ No email — SOS alert will not be sent</p>
                    }
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => handleEdit(c)}
                      className="text-xs border border-gray-200 px-3 py-1.5 rounded-lg text-gray-500 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50 transition-all">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(c._id)}
                      className="text-xs border border-gray-200 px-3 py-1.5 rounded-lg text-gray-500 hover:border-red-300 hover:text-red-500 hover:bg-red-50 transition-all">
                      Delete
                    </button>
                  </div>
                </div>
